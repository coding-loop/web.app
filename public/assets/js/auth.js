...
(function () {
  ...
  CL.auth._errorMessages = {
    ...
    "auth/unauthorized-domain": "Este endereço ainda não está autorizado para fazer login no Firebase.",
    ...
    "auth/emulator-config-failed": "O emulador do Firebase não está disponível. Inicie o Auth Emulator ou abra o site publicado.",
    "auth/internal-error": "O serviço de login encontrou um erro interno. Tente novamente em instantes."
  };
  ...
  CL.auth.init = function () {
    ...
    if (!CL.firebase || !CL.firebase.auth) {
      const error = new Error("Firebase Authentication não foi carregado.");
      error.code = "auth/internal-error";
      if (resolveReady) {
        resolveReady();
        resolveReady = null;
      }
      CL.auth._handleError("init", error);
      throw error;
    }
  ...
  /* AUTH > LOGOUT */
  CL.auth.logout = async function (options) {
    ...
    try {
      ...
      if (options && options.clearLocal && user) CL.api.clearLocalUserData(user.id);
      ...
      CL.auth._handleError("logout", error);
      return false;
    }
  };
