/* =====================================================
   ...
   3) ACRESCENTAR UM MÓDULO A UMA TRILHA
      O fluxo mais simples é adicionar um novo objeto ao final de
      CL.curso.CURSOS.<html|css|js>.modulos, por exemplo:
        {
          id: 'html-modulo-11',      // <cursoId>-modulo-<N>, sempre único
          nome: 'Nome do novo módulo',
          etapas: [ ...5 objetos de etapa, como acima... ]
        }
      Se quiser algo ainda mais fácil, use a função helper
      CL.curso.criarModulo(...) para criar um módulo novo em uma linha.
...
(function () {
  ...
  function criarEtapaPadrao(titulo, texto, missao, codigoInicial, verificar) {
    var etapa = {
      titulo: titulo || 'Etapa',
      texto: texto || '<p>[Placeholder] Conteúdo teórico desta etapa. Substitua este texto pela explicação real.</p>',
      missao: missao || '[Placeholder] Descreva aqui a missão prática que o aluno deve cumprir nesta etapa.'
    };

    if (codigoInicial) etapa.codigoInicial = codigoInicial;
    if (verificar) etapa.verificar = verificar;

    return etapa;
  }

  function criarEtapasPadrao(qtdEtapas, prefixo) {
    var etapas = [];
    for (var i = 0; i < qtdEtapas; i++) {
      etapas.push(criarEtapaPadrao(
        'Etapa ' + (i + 1),
        '<p>[Placeholder] Conteúdo teórico da Etapa ' + (i + 1) + ' do módulo "' + prefixo + '". Substitua este texto pela explicação real.</p>',
        '[Placeholder] Descreva aqui a missão prática que o aluno deve cumprir nesta etapa.'
      ));
    }
    return etapas;
  }

  function criarModulo(cursoId, linguagem, numero, nome, etapas) {
    return {
      id: cursoId + '-modulo-' + numero,
      linguagem: linguagem,
      nome: nome || 'Módulo ' + numero,
      etapas: etapas || criarEtapasPadrao(5, nome || 'Módulo ' + numero)
    };
  }

  function criarCurso(cursoId, nome, linguagem, modulos) {
    return {
      id: cursoId,
      nome: nome,
      linguagem: linguagem,
      modulos: (modulos || []).map(function (modulo, index) {
        if (typeof modulo === 'string') {
          return criarModulo(cursoId, linguagem, index + 1, modulo);
        }
        ...
        return criarModulo(
          ...
          modulo.nome || modulo.titulo || 'Módulo ' + (index + 1),
          modulo.etapas
        );
      })
    };
  ...
  function criarCursoPorTitulos(cursoId, nome, linguagem, titulos) {
    return criarCurso(cursoId, nome, linguagem, titulos.map(function (titulo, index) {
      return {
        numero: index + 1,
        nome: titulo
      };
  ...
  CL.curso.criarEtapaPadrao = criarEtapaPadrao;
  CL.curso.criarEtapasPadrao = criarEtapasPadrao;
  CL.curso.criarModulo = criarModulo;
  CL.curso.criarCurso = criarCurso;
  CL.curso.criarCursoPorTitulos = criarCursoPorTitulos;

  /*
    Padrão escalável para 100+ módulos:
    Para adicionar um módulo novo de forma simples, você pode:
    - usar CL.curso.criarModulo(...) para criar um módulo em uma linha;
    - ou seguir o formato de array abaixo e ir acrescentando objetos.

    Exemplo de uso simples:
      CL.curso.CURSOS.html.modulos.push(
        CL.curso.criarModulo('html', 'html', 11, 'Novo módulo')
      );
  */
...
<body>
  ...
  CL.curso.registrarCurso = function (curso) {
    if (!curso || !curso.id) throw new Error('Curso precisa de um id.');
    if (CL.curso.CURSOS[curso.id]) return CL.curso.CURSOS[curso.id];
    var novoCurso = criarCurso(curso.id, curso.nome || curso.id, curso.linguagem || curso.id, curso.modulos || []);
    novoCurso.descricao = curso.descricao || '';
    novoCurso.logo = curso.logo || '';
    CL.curso.CURSOS[novoCurso.id] = novoCurso;
    CL.curso.ORDEM_CURSOS.push(novoCurso.id);
    return novoCurso;
  };

  CL.curso.registrarModulo = function (cursoId, modulo) {
    var curso = CL.curso.CURSOS[cursoId];
    if (!curso) throw new Error('Curso inexistente: ' + cursoId);
    if (!modulo || !modulo.id) throw new Error('Módulo precisa de um id.');
    var existente = curso.modulos.find(function (item) { return item.id === modulo.id; });
    if (existente) return existente;
    var novoModulo = criarModulo(cursoId, curso.linguagem, modulo.numero || curso.modulos.length + 1, modulo.nome, modulo.etapas || []);
    novoModulo.id = modulo.id;
    curso.modulos.push(novoModulo);
    return novoModulo;
  };

  CL.curso.registrarEtapa = function (moduloId, etapaNumero, dados) {
    var encontrado = CL.curso.encontrarModulo ? CL.curso.encontrarModulo(moduloId) : null;
    if (!encontrado) {
      for (var cursoId in CL.curso.CURSOS) {
        var modulo = CL.curso.CURSOS[cursoId].modulos.find(function (item) { return item.id === moduloId; });
        if (modulo) { encontrado = { modulo: modulo }; break; }
      }
    }
    if (!encontrado || !encontrado.modulo) throw new Error('Módulo inexistente: ' + moduloId);
    var indice = Number(etapaNumero) - 1;
    if (indice < 0 || !Number.isInteger(indice)) throw new Error('Número de etapa inválido: ' + etapaNumero);
    while (encontrado.modulo.etapas.length <= indice) {
      encontrado.modulo.etapas.push(criarEtapaPadrao('Etapa ' + (encontrado.modulo.etapas.length + 1)));
    }
    Object.assign(encontrado.modulo.etapas[indice], dados || {});
    return encontrado.modulo.etapas[indice];
  };
  ...
  CL.curso.aplicarEtapasPorArquivo = function () {
    var lista = CL.curso.etapasPorArquivo || [];
    if (!lista.length) return;

    lista.forEach(function (registro) {
      var cursoId = null;
      var moduloId = registro && registro.moduloId;
      var numeroEtapa = registro && registro.etapaNumero;

      if (!moduloId || !numeroEtapa) return;

      for (var cursoKey in CL.curso.CURSOS) {
        if (!CL.curso.CURSOS.hasOwnProperty(cursoKey)) continue;
        var curso = CL.curso.CURSOS[cursoKey];
        if (!curso || !curso.modulos) continue;

        var modulo = curso.modulos.find(function (item) {
          return item && item.id === moduloId;
        });

        if (!modulo || !modulo.etapas) continue;

        var etapa = modulo.etapas[numeroEtapa - 1];
        if (!etapa) continue;

        cursoId = cursoKey;
        Object.assign(etapa, registro.dados || {});
        break;
      }
    });
  };
  ...
  function moduloConcluido(modulo, progresso) {
    /* Um módulo de planejamento sem etapas não pode ser concluído. */
    if (!modulo || !Array.isArray(modulo.etapas) || modulo.etapas.length === 0) {
      return false;
    }
